...
interface Props {
  // Förifylld starttid när man klickar på en tid i kalendern
  initialDate: Date
  workers: Profile[]
  onClose: () => void
  // Kallas efter lyckad bokning så kalendern kan uppdatera sin data
  onCreated: () => void
}
...
function toLocalInputValue(date: Date): string {
  // Formaterar Date till "YYYY-MM-DDTHH:MM" för datetime-local input
  const pad = (n: number) => String(n).padStart(2, '0')
  return `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}T${pad(date.getHours())}:${pad(date.getMinutes())}`
}

export function CreateBookingModal({ initialDate, workers, onClose, onCreated }: Props) {
  ...
  const [workerName, setWorkerName] = useState('') // fritext om workers-lista är tom
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t border-border shrink-0 gap-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              onClick={e => {
                const form = (e.currentTarget.closest('.flex-col') as HTMLElement)?.querySelector('form')
                form?.requestSubmit()
              }}
              disabled={loading}
